          ...
          {!isProcessing && (
            <div className="flex gap-1 mb-8">
              {steps.map((s, i) => (
                <div
                  key={s}
                  className={`h-1 flex-1 rounded-full ${i <= currentStepIndex ? 'bg-teal-500' : 'bg-gray-200'}`}
                />
              ))}
            </div>
          )}
              ...
              {judgeUsernames.map((u, i) => (
                <div key={i} className="flex gap-2">
                  <select
                    ...
                    className="flex-1 px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent bg-white"
                  >
